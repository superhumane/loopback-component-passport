  ...
  */
  UserIdentity.login = function(provider, authScheme, profile, credentials,
    ...
    userIdentityModel.findOne({where: {
      ...
      let flag = false;
      for(let i = 0; i < options.authorizedList.length; i++) {
        for(let x = 0; x < profile.emails.length; x++) {
          if(options.authorizedList[i] === profile.emails[x].value) flag = true;
        }
      }

      if(!flag) return cb(g.f('email is not part of the authorized list'));
